...
高危请求要求**按住 `1` 达 900ms**（`KIBOARD_HIGH_HOLD_MS`），且不被「全部接受」放行。

计时在 hub 而不是固件：hub 收到 `press` 记时间、`release` 时算真实时长。这么分的原因是
固件在 600ms 就发 `long`，而 600ms 对人手区分不开"点一下"和"按住"——实测有人想短按却
按到了 600ms 以上，系统判成长按并批准了。阈值放 hub 侧还能改配置就生效，不用重新烧板。

到点时黄灯转常亮 + 屏幕提示 `release to accept`，给出过程反馈，人不必靠猜按够了没有。

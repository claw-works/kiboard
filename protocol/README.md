...
| `hello` 的 `caps` 能力声明 | **未实现** |
| `render: "self"`（设备自渲染，hub 不排版） | **未实现**，当前 hub 侧排版 |
| `verbatim` / `summary` 作为协议字段与必显约束 | **未实现**，当前靠 hub 排版时手工遵守 |
| 角色化认证（client 不得提交裁决） | **未实现**，当前 `/device` 单一共享 token |
| `decided_by` / `verified_by` 入审计 | **未实现** |
| `confirm.method` 分级与 `confirm_verifiable` | **未实现**，当前只有物理键的 hold |
| `protocol_version` 协商 | **未实现** |

改造顺序建议：先做 `hello` + `verbatim` + 自渲染（拿现有 c3-keypad 跑通，目标是能删掉
hub 里的 `keymap.rs`、`approval.rs` 不再拼屏幕字符串），再做角色化认证，最后上第二个设备方案。
